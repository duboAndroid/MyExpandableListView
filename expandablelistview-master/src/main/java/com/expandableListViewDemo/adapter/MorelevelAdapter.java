...
public class MorelevelAdapter extends BaseExpandableListAdapter {
    ...
    public View getGroupView(final int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        // 没有子章节，父章节前面的展开箭头隐藏
        if (null == this.list.get(groupPosition).getChildren() || 0 == this.list.get(groupPosition).getChildren().size()) {
    ...
    public View getChildView(final int groupPosition, final int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
        if (null != list.get(groupPosition).getChildren().get(childPosition)
            ...
            if (isLastChild
                    && childPosition == this.list.get(groupPosition).getChildren().size() - 1// 最后一个child
                    && (groupPosition == this.list.size() - 1 || "00".equals(this.list.get(groupPosition)// 父类也是最后一个
                    .getParent_code()))) {
